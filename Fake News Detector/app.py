...
# ⿡ Set a writable folder for NLTK data
...
# ⿢ Download required corpora before importing TextBlob
required_corpora = ["wordnet", "omw-1.4", "punkt", "stopwords", "averaged_perceptron_tagger"]
for pkg in required_corpora:
    try:
        if pkg == "punkt":
...
# ------------------- NLTK Setup (Fix LookupError) -------------------
import nltk

# Use writable folder for Streamlit Cloud
NLTK_DATA_DIR = "/tmp/nltk_data"
os.makedirs(NLTK_DATA_DIR, exist_ok=True)
if NLTK_DATA_DIR not in nltk.data.path:
    nltk.data.path.append(NLTK_DATA_DIR)
...
# Required packages
nltk_packages = ["punkt", "wordnet", "omw-1.4", "stopwords", "averaged_perceptron_tagger"]
...
def get_lime_explanation(headline):
    if explainer is None: return "LIME not available.", None
    try:
...
def save_confidence_timeline(headline,label,confidence):
    ...
    if os.path.exists(CONF_HISTORY):
        ...
        df = data
    try: df.to_csv(CONF_HISTORY,index=False)
...
def analyze_headline_ml_online(headline):
    ...
    if is_verified: explanation_text += f"Verified in trusted sources: {', '.join(trusted_links)}. "
    explanation_text += f"Influential words: {lime_words}"
    save_confidence_timeline(headline,final_label,final_confidence)
    return {"headline":headline,"label":final_label,"confidence":final_confidence,
            "sentiment":sentiment,"readability":readability,
            "explanation":explanation_text,"lime_exp":exp,"trusted_links":trusted_links}

def analyze_batch_csv(uploaded_file):
    try:
        df=pd.read_csv(uploaded_file)
        df.columns=df.columns.str.lower()
        if "headline" not in df.columns: st.error("CSV must contain 'headline' column."); return None
        results=[]
        for idx,row in df.iterrows():
            res = analyze_headline_ml_online(str(row["headline"]))
            results.append({"headline":res["headline"],"label":res["label"],
                            "confidence":res["confidence"],"explanation":explain_simple(res["label"],res["confidence"])})
        return pd.DataFrame(results)
    except Exception as e: st.error(f"Error processing CSV: {e}"); return None
...
# Header
st.markdown("""<div style="background:linear-gradient(90deg,#ff4b4b,#ff6f91);padding:20px;border-radius:15px;text-align:center;">
...
def get_trending_topics(headlines):
    if nlp is None:  # spaCy model not loaded
...
with tabs[3]:
    ...
    # Make sure this function is defined above this line
