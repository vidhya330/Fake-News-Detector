...
# --- Fix for missing NLTK resources (safe for Streamlit Cloud and local use) ---
import nltk
import os

# Set a writable nltk_data path (important for Streamlit)
...
# LIME import may be heavy — import inside try so app can still start if missing
try:
...
# ------------------- Robust NLTK setup (Windows-friendly) -------------------
# Use a local nltk_data folder (in working directory) so we don't hit roaming permission issues
nltk_data_dir = os.path.join(os.getcwd(), "nltk_data")
os.makedirs(nltk_data_dir, exist_ok=True)
if nltk_data_dir not in nltk.data.path:
    nltk.data.path.append(nltk_data_dir)

# Packages we need for TextBlob and NLTK-based features
nltk_packages = {
...
for pkg, resource_path in nltk_packages.items():
    try:
        ...
        try:
            # Attempt to download into the local nltk_data_dir; catch permission errors
            nltk.download(pkg, download_dir=nltk_data_dir, quiet=True)
        except PermissionError:
            # If permission error, ignore and continue (we'll degrade gracefully)
            pass
        except Exception:
            # Any other download error — ignore to keep app alive; features relying on pkg may degrade
            pass

# Try to import stopwords; if fail, use empty set to avoid crashes
try:
...
try:
    ...
    # Try to download the small model into the environment; if that fails, continue with nlp=None
    try:
        import spacy.cli
...
def try_load_models():
    global ml_model, vectorizer, explainer
    try:
        ml_model = joblib.load("fake_news_model.pkl")
    except Exception as e:
        model_load_errors.append(f"Couldn't load fake_news_model.pkl: {e}")
        ml_model = None
    try:
        vectorizer = joblib.load("vectorizer.pkl")
    except Exception as e:
        model_load_errors.append(f"Couldn't load vectorizer.pkl: {e}")
        vectorizer = None

    # create Lime explainer only if Lime available and model exists
    if LimeTextExplainer is not None and ml_model is not None:
        try:
            explainer = LimeTextExplainer(class_names=["Fake", "Real"])
        except Exception as e:
            model_load_errors.append(f"Couldn't create LIME explainer: {e}")
            explainer = None
    else:
        explainer = None
...
def ml_predict_proba(texts):
    """Return array shape (n,2) with [prob_fake, prob_real] for each input text.
       If model/vectorizer not available, raise."""
    if ml_model is None or vectorizer is None:
        raise RuntimeError("ML model or vectorizer not loaded.")
...
def get_sentiment(text: str) -> float:
    # Ensure TextBlob dependencies exist; attempt to download quietly if missing
    for pkg in ("punkt", "wordnet"):
        try:
            nltk.data.find(f"corpora/{pkg}") if pkg != "punkt" else nltk.data.find("tokenizers/punkt")
        except LookupError:
            try:
                nltk.download(pkg, download_dir=nltk_data_dir, quiet=True)
            except Exception:
...
def get_trending_topics(headlines):
    ...
    if not words.strip() or nlp is None:
        ...
        try:
            ...
            for entry in feed.entries:
                if headline.lower() in entry.title.lower():
                    found_sources.append(getattr(entry, "link", ""))
        except Exception:
            continue
...
def get_lime_explanation(headline):
    if explainer is None:
        return "LIME not available.", None
    try:
        ...
        return "LIME explanation failed.", None

def save_confidence_timeline(headline, label, confidence):
    data = pd.DataFrame([[time.strftime("%H:%M:%S"), headline[:40], label, confidence]], columns=["Time", "Headline", "Label", "Confidence"])
...
def analyze_headline_ml_online(headline):
    ...
    if hasattr(ml_model, "decision_function"):
        ...
        pred_prob = float(expit(score))
    else:
        pred_prob = float(ml_model.predict_proba(vector)[0][1])
...
# Top-level model errors (if any) — show to developer/users
if model_load_errors:
    st.sidebar.warning("Model loading warnings (check logs):")
    for err in model_load_errors:
        st.sidebar.write("- " + str(err))
...
with tabs[0]:
    ...
    if st.button("🔍 Analyze Headline"):
        if not headline_input.strip():
            st.info("Please enter a headline to analyze.")
        else:
            if ml_model is None or vectorizer is None:
                st.error("ML model or vectorizer not loaded. Upload fake_news_model.pkl and vectorizer.pkl in the app root.")
            else:
                try:
                    ...
                    if entities:
                        ...
                        st.info("No entities found or spaCy model not available.")

                    # Wordcloud
                    wc = generate_wordcloud(headline_input)
                    if wc:
                        st.image(wc, caption="WordCloud", use_column_width=True)

                    # Radar Chart
                    labels = ["Confidence", "Sentiment", "Readability"]
                    values = [ml_result["confidence"] / 100.0, abs(ml_result["sentiment"]), ml_result["readability"] / 100.0]
                    fig = go.Figure(
...
with tabs[1]:
    ...
    if uploaded_file:
        if ml_model is None or vectorizer is None:
            st.error("ML model or vectorizer not loaded. Batch analysis requires these files in the app root.")
        else:
...
with tabs[2]:
    ...
    if os.path.exists(CONF_HISTORY):
        try:
            ...
            st.error(f"Could not load confidence history: {e}")
    else:
...
with tabs[3]:
    ...
    if not top.empty:
        ...
        st.info("No trending topics found or spaCy model not available.")
    st.markdown("</div>", unsafe_allow_html=True)
